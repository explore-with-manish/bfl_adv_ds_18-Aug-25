...
st.title("📈 Charts & Maps using Streamlit")
st.caption(
...
st.write(
    "**line_chart / area_chart / bar_chart / scatter_chart** accept DataFrames directly."
)

pivot = df_view.pivot_table(
    index="date", columns="city", values=metric, aggfunc="sum"
).sort_index()
...
st.caption(
    "Tip: Put cities on columns and dates on the index for quick multi-series charts."
)
...
with k1:
    st.metric(
        f"Total {metric.capitalize()} (last {days_back}d)",
        f"{total_metric:,.0f}" if metric == "orders" else f"{total_metric:,.2f}",
    )
with k2:
    st.metric(
        f"Today {metric.capitalize()}",
        f"{today_metric:,.0f}" if metric == "orders" else f"{today_metric:,.2f}",
